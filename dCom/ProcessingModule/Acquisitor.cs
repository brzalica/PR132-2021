...
namespace ProcessingModule
{
    ...
    public class Acquisitor : IDisposable
	{
		...
		private void Acquisition_DoWork()
		{
            ...
            while (true) // akvizicija se vrsi sve dok radi program
            {
                acquisitionTrigger.WaitOne();
                foreach (IConfigItem item in config)
                {
                    item.SecondsPassedSinceLastPoll++; // pokazuje koliko je vremena proteklo od poslednje akvizicije
                    if (item.SecondsPassedSinceLastPoll == item.AcquisitionInterval) // proveravamo da li je jednako sa akvizicionim intervalom podesenim u konfiguraciji
                    {
